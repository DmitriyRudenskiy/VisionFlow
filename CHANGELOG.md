...
#### Added
- В интерфейс `StoragePort` добавлен абстрактный метод `get_all_files(path: Path, recursive: bool = True) -> list[Path]`, предназначенный для получения **всех** файлов в директории без фильтрации по расширениям изображений.
- Добавлены документирующие комментарии к методам `scan_directory` и `get_all_files`, явно указывающие на различие в поведении (только изображения vs все файлы).

#### Changed
- Файл `src/application/ports.py` обновлён: расширен интерфейс `StoragePort` новым методом.

#### Fixed
- Устранено архитектурное несоответствие: шаги `FlattenDirectoriesStep` и `PrepareImagesStep` требовали обработки любых файлов (включая не‑изображения), однако использовали метод `scan_directory`, который возвращал только изображения. Теперь для таких сценариев введён отдельный метод `get_all_files`.

### Примечание
- Реализация метода `get_all_files` в адаптере `FileSystemStorage` и переключение упомянутых шагов на новый метод **не выполнены** в рамках данного изменения — это следующие необходимые шаги.
- Остальные шаги (дедупликация, AI‑обработка) по‑прежнему корректно используют `scan_directory`, так как ожидают только изображения.

## [K2.6 Thinking]

### Исправлено (Fixed)

- **SmartCropStep**: кропнутое изображение теперь корректно заменяет оригинал вместо сохранения под случайным именем временного файла.
- **PipelineAggregate**: исправлена логика финализации статуса пайплайна. При не-критичном падении шага (`critical=False`) агрегат больше не застревает в статусе `RUNNING`.
- **FileSystemStorage.move_file()**: устранена опасная операция `shutil.rmtree` при конфликте с директорией-назначением. Теперь выбрасывается `IsADirectoryError`.
- **PipelineOrchestrator**: добавлена проверка `future.done()` перед отменой задач в параллельной группе, предотвращающая попытку отмены уже завершённых future.
- **ColorExtractorClient**: добавлена обработка исключений при открытии повреждённых/невалидных изображений (`Image.open`).

### Изменено (Changed)

- **ColorPaletteExtractionStep**: унифицировано именование поля в JSON-выводе — теперь везде используется `percentage` (вместо смешения `percent`/`percentage`).
- **BatchFileProcessingStep**: добавлена ранняя валидация `source_path`. Невалидный путь возвращает `FAILED` с детальным сообщением вместо молчаливого нулевого результата.
- **FlattenDirectoriesStep**: добавлена валидация `source_path` на входе.
- **PipelineOrchestrator.execute()**: добавлена валидация существования и типа `source_path` перед запуском пайплайна.

### Удалено (Removed)

- **SAM3Client**: удалён неиспользуемый мёртвый код вычисления padding (`pad_w`, `pad_h`) в методе `_crop_to_square`.

### Добавлено (Added)

- **Тесты (`test_pipeline_steps.py`)**:
  - Актуализирован `test_smart_crop_handles_temp_file` под исправленную логику замены оригинала.
  - Добавлен `test_smart_crop_reports_errors` для проверки обработки ошибок сегментации.
- **Тесты (`test_color_palette_extraction.py`)**: обновлена проверка поля `percentage` с обратной совместимостью для старых эталонных JSON (`percent`).

### Безопасность (Security)

- **FileSystemStorage**: устранён риск случайного рекурсивного удаления директории при перемещении файла в существующий путь с совпадающим именем директории.
